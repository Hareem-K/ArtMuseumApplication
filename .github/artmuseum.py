...
2 - Modify existing information in the database """) 
    ...
    2 - Collections """) 
        ...
        if data_subinput == '1':
            ...
            art = cur.fetchall()
            print("Updated Artist Table:")
            print(tabulate(art, headers=['Artist Name', 'Birth', 'Death', 'Country of Origin', 'Epoch', 'Main style', 'Artist Description'], tablefmt='rounded_outline', missingval="N/A", numalign="left"))
        ...
        if data_subinput == '2':
            ...
            cur.execute("INSERT INTO COLLECTIONS (Collection_name, Collection_Type, Collection_Description, Cl_Address, Phone, Contact_person) VALUES (%s, %s, %s, %s, %s, %s)", (collection_name, Collection_type, Collection_description, C_Address, Phone, Contact_Person))
            cur.execute("SELECT * FROM COLLECTIONS")
            collection = cur.fetchall()
            print("Updated Collections Table:")
            print(tabulate(collection, headers=['Col name', 'Col Type', 'Col Description', 'Address', 'Phone', 'Contact'], tablefmt='rounded_outline', missingval="N/A", numalign="left"))

    if data_input == '2':
        print("""Available databases:
    1 - Artist
    2 - Collections """) 
        data_subinput = input("Please choose a database to make changes: ")

        if data_subinput == '1':

            cur.execute("SELECT * FROM ARTIST")
            result = cur.fetchall()
            print(tabulate(result, headers=['Artist_Name', 'Date_Born', 'Date_Died', 'Country_of_Origin', 'Epoch', 'Main_style', 'Artist_Description'], tablefmt='rounded_outline', missingval="N/A", numalign="left"))
            print("\n")

            subselection = input("Please input artist name as shown in table: ")
            change_info = input("Enter a column name that you would like to change for this artist as shown in the table: ")
            change_made = input("Enter the new value: ") 

            query = "UPDATE ARTIST SET {} = {} WHERE Artist_name =(%s)" .format(change_info, change_made)
            cur.execute(query, (subselection,))
        ...
        elif data_subinput == '2':
            cur.execute("SELECT * FROM COLLECTIONS")
            result = cur.fetchall()
            print(tabulate(result, headers=['Collection_Name', 'Collection_Type', 'Collection_Description', 'Cl_Address', 'Phone', 'Contact_person'], tablefmt='rounded_outline', missingval="N/A", numalign="left"))
            print("\n")

            subselection = input("Please input Collection name as shown in table: ")
            change_info = input("Enter a column name that you would like to change for this collection as shown in the table: ")
            change_made = input("Enter the new value: ") 

            query = "UPDATE COLLECTIONS SET {} = {} WHERE Collection_Name =(%s)" .format(change_info, change_made)
            cur.execute(query, (subselection,))

            print("Updated Collections Table:")
            cur.execute("SELECT * FROM COLLECTIONS")
            result = cur.fetchall()
            print(tabulate(result, headers=['Col Name', 'Col Type', 'Col Description', 'Address', 'Phone', 'Contact'], tablefmt='rounded_outline', missingval="N/A", numalign="left"))
            print("\n")
...
def borrow():
    ...
    print(tabulate(borrowed, headers=['Borrowed Art Objects in Art Museum', 'ID#'], tablefmt='rounded_outline', numalign="left"))
...
def pcollection():
    ...
    print(tabulate(b_collect, headers=['Permanenet Art Objects in Art Museum', 'ID#'], tablefmt='rounded_outline', numalign="left"))
...
def exhibition():
    ...
    b_collect = cur.fetchall()
    print(tabulate(b_collect, headers=['Exhibitions in Art Museum', 'Art ID#'], tablefmt='rounded_outline', numalign="left"))
